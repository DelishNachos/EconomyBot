...
energy_loss_per_tick = 10  # base energy loss per tick
...
def simulate_single_race(horses, track, track_data, corner_indices):
    ...
    while max(positions.values()) < track_length:
        for h in horses:
            ...
            energy_loss = energy_loss_per_tick * (1 - (h["stamina"] / max_stat))
...
def main():
    ...
    for sim in range(NUM_SIMULATIONS):
        winner = simulate_single_race(horses, track, track_data, corner_indices)
        wins[winner] += 1
    ...
    print("\n=== Simulation Results ===")
    for horse_id, count in wins.items():
        percent = (count / NUM_SIMULATIONS) * 100
        print(f"{horse_id}: {count} wins ({percent:.2f}%)")
