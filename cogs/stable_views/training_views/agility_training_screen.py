...
class AgilityTrainingView(discord.ui.View):
    def __init__(self, user_id, horse):
        super().__init__(timeout=300)
        self.user_id = user_id
        self.horse = horse

        can_buy = db.get_balance(user_id) >= training_calculator.calculate_cost(horse['agility'])
...
class StartButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        response = await stable_view_factory.agility_game_screen(interaction, interaction.user.id, self.horse)
        await interaction.response.edit_message(
            content=response["content"],
            embed=response["embed"],
            view=response["view"],
